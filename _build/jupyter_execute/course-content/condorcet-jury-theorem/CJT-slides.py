# Introduction to the Condorcet Jury Theorem
...
A set of voters, or experts, $V=\{1, 2, 3, \ldots\}$


A binary set of alternatives: $\{\mbox{convict}, \mbox{acquit}\}$, $\{\mbox{abolish}, \mbox{keep}\}$, $\{0,1\}$, $\ldots$


A random variable (called the **state**)  $\mathbf{x}$ whose values range over the two alternatives. 


Random variables  $\mathbf{v}_1, \mathbf{v}_2, \ldots$ represeting the votes for individuals $1, 2, \ldots$


$R_i$ is the event that $i$ votes correctly is then defined as the event that $v_i$ coincides with
the state

**Unconditional independence (UI)**: The correctness events $R_1, R_2, \ldots$ are (unconditionally) independent.


**Unconditional competence (UC)**: The (unconditional) correctness probability
$p = P(R_i)$, the (unconditional) competence, (i) exceeds $\frac{1}{2}$ and (ii) is the same for
each voter $i$.
...
$$\sum_{h=m}^n {n\choose h} *p^h*(1-p)^{n-h}$$
...
@interact(num_voters=(1, 200, 1), prob=(0,1,0.1))
def probability_majority_is_correct_latex(num_voters=100,prob=0.51):
    answer = probability_majority_is_correct(num_voters=num_voters, prob=prob)
    latex_str = r'\sum_{k=' + str(math.ceil(num_voters/2 +1)) + '}^{' + str(num_voters) + '}'
    latex_str = latex_str + '{' + str(num_voters) + '\choose k }*'  + str(prob) + '^k * ' + str(1-prob) + '^{' + str(num_voters) + '-k} = ' + str(answer)
...
Christian List and Robert E Goodin. Epistemic democracy: Generalizing the condorcet jury theorem. Journal of
